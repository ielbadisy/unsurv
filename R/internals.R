...
.check_inputs <- function(S, times) {
  ...
  if (length(times) < 2) stop("'times' must have length >= 2.")
  ...
  if (nrow(S) < 2) stop("Need at least 2 rows in 'S' to cluster.")
...
## ---- Stability via ARI over resamples (internal) ----
...
## ---- Plot ARI distribution (internal) ----
plot_stability <- function(stab){
  ...
  if (is.list(stab) && !is.null(stab$aris)) {
    ...
    aris <- stab
  } else stop("Pass the list returned by unsurv_stability(..., return_distribution=TRUE).")
...
autoplot.unsurv <- function(object, ...) {
  ...
  # Build a long data.frame of medoid curves (no dplyr/tidyr needed)
  times <- object$times
  med <- object$medoids
  K <- nrow(med)
